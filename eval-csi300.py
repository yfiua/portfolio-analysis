...
def main(args):
    ...
    # download CSI 300 data
    df_csi300 = yf.download('000300.SS', start_date, end_date)

    # calculate returns
    df_returns = df_data['Adj Close'][symbols].pct_change()
    df_returns_csi300 = df_csi300['Adj Close'].pct_change()
    df_returns_csi300.iloc[0] = 0   # set first value to 0

    # calculate portfolio returns based on weights
    df_portfolio_returns = (df_returns * weights).sum(axis=1)

    print(df_portfolio_returns)
    print(df_returns_csi300)

    # calculate portfolio value
    df_portfolio_value = (1 + df_portfolio_returns).cumprod()
    df_portfolio_value_csi300 = (1 + df_returns_csi300).cumprod()
    ...
    plt.plot(df_portfolio_value.index, df_portfolio_value, label='夹头机', color='red')
    plt.plot(df_portfolio_value_csi300.index, df_portfolio_value_csi300, label='沪深300')
    print(df_portfolio_value.index)
    print(df_portfolio_value_csi300.index)
    print(df_portfolio_value)
    print(df_portfolio_value_csi300)
